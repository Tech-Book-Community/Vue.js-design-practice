...
const currentState = computed(() => {
  ...
  if (clicks >= 2) {
    ...
    // newEnd(p-3) at y=240, oldEnd(p-3) at y=160
  ...
  if (clicks >= 5) {
    comparing = ['newEnd', 'oldEnd']
    // newEnd(p-2) at y=160, oldEnd(p-2) at y=80
    connection = { fromY: 160 + 40, toY: 80 + 40, type: 'match', show: true }
    domNodes = [
      { key: 'p-1', y: 0, highlight: false },
      { key: 'p-2', y: 80, highlight: true },
      { key: 'p-3', y: 160, highlight: false },
    ]
    message = '②：newEnd(p-2) vs oldEnd(p-2) ✅ 命中！'
  }
  ...
  if (clicks >= 8) {
    comparing = ['newEnd', 'oldEnd']
    // newEnd(p-1) at y=80, oldEnd(p-1) at y=0
    ...
    message = '②：newEnd(p-1) vs oldEnd(p-1) ✅ 命中！'
  }
  ...
  if (clicks >= 9) {
    ...
    oldStartIdx = 1
    oldEndIdx = 0 // oldEndIdx < oldStartIdx，舊節點處理完了
    newEndIdx = 0
    comparing = []
    connection.show = false
    message = '尾尾相同，不移動，只 patch'
  ...
  if (clicks >= 10) {
    ...
    displayNewEndIdx = 0
    comparing = ['newStart']
    message = 'oldEndIdx < oldStartIdx，但 newStartIdx <= newEndIdx'
  }

  if (clicks >= 11) {
    // 發現還有新節點沒處理
    comparing = ['newStart']
    message = '還有新節點 p-4 沒處理！需要掛載'
  }
  ...
  if (clicks >= 13) {
    ...
    message = '✓ 完成！DOM: [p-4, p-1, p-2, p-3]'
  }
