...
const currentState = computed(() => {
  ...
  let message = '初始狀態：DOM [p-1, p-2, p-3]'
  let step = '初始狀態'
  let comparing: string[] = []
  let connection = {
    fromY: 0,
    toY: 0,
    type: 'start' as 'start' | 'end' | 'match' | 'search' | 'notfound',
    show: false,
  }
  const searchHighlight = -1
  ...
  if (clicks >= 1) {
    ...
    // newStart(p-4) at y=0, oldStart(p-1) at y=0
  ...
  if (clicks >= 2) {
    ...
    // newEnd(p-2) at y=240, oldEnd(p-3) at y=160
    connection = { fromY: 240 + 40, toY: 160 + 40, type: 'end', show: true }
    message = '②：newEnd(p-2) vs oldEnd(p-3) ❌'
  }

  if (clicks >= 3) {
    comparing = ['newEnd', 'oldStart']
    // newEnd(p-2) at y=240, oldStart(p-1) at y=0
    connection = { fromY: 240 + 40, toY: 0 + 40, type: 'end', show: true }
    message = '③：newEnd(p-2) vs oldStart(p-1) ❌'
  }

  if (clicks >= 4) {
    comparing = ['newStart', 'oldEnd']
    // newStart(p-4) at y=0, oldEnd(p-3) at y=160
    connection = { fromY: 0 + 40, toY: 160 + 40, type: 'end', show: true }
    message = '④：newStart(p-4) vs oldEnd(p-3) ❌'
  }
  ...
  if (clicks >= 5) {
    ...
    message = '四步都沒命中！拿 newStart(p-4) 去舊節點中查找...'
  }
  ...
  if (clicks >= 6) {
    ...
    comparing = ['newStart']
    searchNotFound = true
    message = '在舊節點中找不到 p-4！這是新增節點！'
  }

  if (clicks >= 7) {
    // 立即掛載 p-4
    domNodes = [
      { key: 'p-4', y: 0, highlight: false, mounting: true },
    ...
    message = '掛載 p-4 到 oldStartVNode.el (p-1) 之前'
  }
  ...
  if (clicks >= 8) {
    ...
    comparing = ['newStart', 'oldStart']
    // newStart(p-1) at y=80, oldStart(p-1) at y=0
    connection = { fromY: 80 + 40, toY: 0 + 40, type: 'match', show: true }
    domNodes = [
      { key: 'p-4', y: 0, highlight: false },
  ...
  if (clicks >= 10) {
    ...
    comparing = ['newStart', 'oldStart']
    // newStart(p-3) at y=160, oldStart(p-2) at y=80
    connection = { fromY: 160 + 40, toY: 80 + 40, type: 'start', show: true }
    message = '①：newStart(p-3) vs oldStart(p-2) ❌'
  }
  ...
  if (clicks >= 12) {
    comparing = ['newEnd', 'oldStart']
    // newEnd(p-2) at y=240, oldStart(p-2) at y=80
    connection = { fromY: 240 + 40, toY: 80 + 40, type: 'match', show: true }
    domNodes = [
      { key: 'p-4', y: 0, highlight: false },
      { key: 'p-1', y: 80, highlight: false },
      { key: 'p-2', y: 160, highlight: true },
      { key: 'p-3', y: 240, highlight: false },
    ]
    message = '③：newEnd(p-2) vs oldStart(p-2) ✅ 命中！'
  }
  ...
  if (clicks >= 13) {
    ...
    domNodes = [
      ...
      { key: 'p-1', y: 80, highlight: false },
      { key: 'p-3', y: 160, highlight: false },
      { key: 'p-2', y: 240, highlight: true },
    ]
    oldStartIdx = 2
    newEndIdx = 2
    comparing = []
    connection.show = false
    message = '舊頭 = 新尾，移動 p-2 到尾部'
  }
  ...
  if (clicks >= 14) {
    ...
    displayNewEndIdx = 2
    comparing = ['newStart', 'oldStart']
    // newStart(p-3) at y=160, oldStart(p-3) at y=160
    connection = { fromY: 160 + 40, toY: 160 + 40, type: 'match', show: true }
    domNodes = [
      { key: 'p-4', y: 0, highlight: false },
      { key: 'p-1', y: 80, highlight: false },
      { key: 'p-3', y: 160, highlight: true },
      { key: 'p-2', y: 240, highlight: false },
    ]
  ...
  if (clicks >= 16) {
    ...
    displayNewEndIdx = 2
    message = '✓ 完成！DOM: [p-4, p-1, p-3, p-2]'
  }

  const getOldHighlight = (idx: number) => {
    if (searchHighlight === idx)
      return 'search'
...
<template>
  <div class="diff-animation-add-immediate">
        ...
        <div class="layers-grid">
          ...
          <div class="layer-column">
            ...
            >
              ...
              >
                <rect
                  ...
                  :class="{
                    ...
                    'highlight-end': node.highlightType === 'end',
                    'processed': node.processed,
                    'search-not-found': currentState.searchNotFound,
                  }"
                  rx="6"
                />
                <text
              ...
              <!-- Not found overlay -->
              <g v-if="currentState.searchNotFound">
                <text
                  x="65"
                  y="280"
...
.dom-node-group.mounting {
  animation: mount-fade-in 0.8s ease-out;
}
...
/* Search not found - red pulse */
.old-node.search-not-found {
  @apply stroke-red-400;
  animation: not-found-pulse 0.6s ease-in-out infinite;
}
...
.dom-node.mounting-node {
  @apply fill-emerald-900/50 stroke-emerald-400 stroke-3;
  filter: drop-shadow(0 0 12px rgba(52, 211, 153, 0.8));
  animation: mount-glow 1.5s ease-in-out infinite;
}
